...
*/
/*
 * CarMeter 自定义 U-Boot autoboot:
 * 直接从 eMMC (mmc 1:1) 加载 zImage + 适配 Pro 板的 imx6ull-mmc-npi.dtb,
 * 完全跳过 distro_bootcmd / findfdt / /uEnv.txt 等依赖 env var 的链路。
 * 优势: fresh mfgtools 烧写完成后首次启动即直接进 rootfs,
 *       无需手动 setenv / saveenv。
 *
 * 依赖 (均由 CONFIG_EXTRA_ENV_SETTINGS 默认提供, 无需 env 已保存即可用):
 *   mmcdev   = 1   (eMMC USDHC2, 来自 CONFIG_SYS_MMC_ENV_DEV=1)
 *   mmcpart  = 1   (来自 CONFIG_SYS_MMC_IMG_LOAD_PART=1)
 *   image    = zImage
 *   mmcboot  = 已定义 (调用 run mmcargs 传 root=/dev/mmcblk1p2, 然后 bootz)
 *   fdt_addr = 0x83000000
 */
#undef CONFIG_BOOTCOMMAND
#define CONFIG_BOOTCOMMAND \
	"setenv fdt_file imx6ull-carmeter-pro.dtb; " \
	"mmc dev ${mmcdev}; " \
	"load mmc ${mmcdev}:1 ${loadaddr} ${image}; " \
	"load mmc ${mmcdev}:1 ${fdt_addr} imx6ull-carmeter-pro.dtb; " \
	"run mmcboot"
#endif
